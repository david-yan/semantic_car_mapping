...
def eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        cnt += 1
        sample_id, pts_rect, pts_features, pts_input, pts_lidar = \
            data['sample_id'], data['pts_rect'], data['pts_features'], data['pts_input'], data['pts_lidar']
        ...
        # save data to pickle
        if len(pred_boxes3d_selected) != 0:
            print(pred_boxes3d_selected, scores_selected)
            pickle_dict = {'scan_cam' : pts_input, 'scan_lidar': pts_lidar, 'pred_box': pred_boxes3d_selected, 'scores': scores_selected}
            with open('../data/PENN/KITTI/' + str(cur_sample_id) + '.pkl', 'wb') as handle:
