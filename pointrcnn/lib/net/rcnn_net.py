    ...
    def forward(self, input_data):
        ...
        if cfg.RCNN.ROI_SAMPLE_JIT:
            if self.training:
                ...
                pts_feature = torch.cat((pts_extra_input, rpn_features), dim=2) # (B, N, C_RPN + 2)
                pooled_features, pooled_empty_flag = \
                        roipool3d_utils.roipool3d_gpu(rpn_xyz, pts_feature, batch_rois, cfg.RCNN.POOL_EXTRA_WIDTH,
                                                      sampled_pt_num=cfg.RCNN.NUM_POINTS)   # (B, M, 512, 3+C_RPN+2), 512 is the number of points within the box, and +3 for xyz
                ...
                pts_input = pooled_features.view(-1, pooled_features.shape[2], pooled_features.shape[3])    # (B*M, 512, 3+C_RPN+2)
        else:
        ...
        if cfg.RCNN.USE_RPN_FEATURES:
            xyz_input = pts_input[..., 0:self.rcnn_input_channel].transpose(1, 2).unsqueeze(dim=3)  # (B*M, 5, 512, 1)
            xyz_feature = self.xyz_up_layer(xyz_input)  # (B*M, 128, 512, 1)

            rpn_feature = pts_input[..., self.rcnn_input_channel:].transpose(1, 2).unsqueeze(dim=3) # (B*M, C_RPN, 512, 1)

            merged_feature = torch.cat((xyz_feature, rpn_feature), dim=1)   # (B*M, 128 + C_RPN, 512, 1)
            merged_feature = self.merge_down_layer(merged_feature)          # (B*M, 128, 512)
            l_xyz, l_features = [xyz], [merged_feature.squeeze(dim=3)]      # (B*M, 512, 3), (B*M, 128, 512)
        else:
        ...
        # l_features[-1] has shape (M, 512, 1)
        rcnn_cls = self.cls_layer(l_features[-1]).transpose(1, 2).contiguous().squeeze(dim=1)  # (B*M, 1 or 2)
        rcnn_reg = self.reg_layer(l_features[-1]).transpose(1, 2).contiguous().squeeze(dim=1)  # (B*M, C)
