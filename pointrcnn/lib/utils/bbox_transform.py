...
def decode_bbox_target(roi_box3d, pred_reg, loc_scope, loc_bin_size, num_head_bin, anchor_size,
    ...
    if get_ry_fine: # for RCNN second stage
    ...
    if roi_box3d.shape[1] == 7: # content in this if is for rcnn. the proposal_layer for rpn only have 3 dimension for roi_box3d, which is the coordinate of the point of interest
        roi_ry = roi_box3d[:, 6]
